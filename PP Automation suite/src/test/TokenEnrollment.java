...
import pom.TermsAndConditions_Main;


public class TokenEnrollment {

	public static WebDriver driver;
	public TokenEnrollment(WebDriver driver) throws Exception
	{
		TokenEnrollment.driver=driver;
		//this.login();
		this.tokenEnr();
		Thread.sleep(4000);
		new ExcelOperations().WriteToExcel(GlobalValues.ExecutionController, "ExecutionController", 1, 5, "Passed--Logged Into application");
		this.Logout();
	}
	...
	public void tokenEnr() throws Exception
	{
		...
		PageFactory.initElements(TokenEnrollment.driver, terms);
		...
		if(driver.getTitle().equals("Google Authenticator - Patient Portal"))
			{if(bln_Status & bln_textComparision & bln_Buttons)
			{
				...
				Thread.sleep(3000);
				if(driver.getTitle().equals("Dashboard Page is not displayed"))
